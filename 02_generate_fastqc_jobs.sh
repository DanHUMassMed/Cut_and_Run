...
JOB_DIR="jobs/02_fastqc"
FASTQC_SCRIPT="02_fastqc.sh"
...
if [[ ! -f "$FASTQC_SCRIPT" ]]; then
  echo "ERROR: FastQC script not found: $FASTQC_SCRIPT"
  exit 1
fi
...
for R1 in "$TRIM_DIR"/*_R1_*_paired.fq.gz; do
    ...
    # Job script path
    JOB_SCRIPT="$JOB_DIR/job_fastqc_${SAMPLE}.sh"
...
echo
echo "All FastQC job scripts created in: $JOB_DIR"
echo "Submit with:"
